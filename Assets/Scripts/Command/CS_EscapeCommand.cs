...
public class CS_EscapeCommand : IBattleCommand
{
    ...
    public void Execute(CS_CharacterState user, CS_CharacterState target)
    {
        ...
        CS_BattleStateMachine battleStateMachine = GameObject.FindAnyObjectByType<CS_BattleStateMachine>();

        battleStateMachine.context.result = CSE_BattleResult.Escape; // 逃げることに成功した場合の結果を設定
    }
}